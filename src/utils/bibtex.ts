...
export interface Publication {
  ...
  institution?: string;
  /** Custom field: featured = {true} marks a publication for the Selected works section */
  featured?: boolean;
  /** Custom field: featuredNote provides 2-3 sentence context for featured works */
  featuredNote?: string;
  /**
   * For @misc entries: distinguishes posters, talks, etc. from general preprints.
   * Populated from the BibTeX `type` field: `@misc{key, type = {poster}, ...}`.
  ...
  for (const raw of parsed.entries as ParsedEntry[]) {
    try {
      ...
      const pub: Publication = {
        ...
        featured: /^(true|yes|1)$/i.test(fieldStr(fields, "featured")),
        featuredNote: cleanLatex(fieldStr(fields, "featurednote") || fieldStr(fields, "featured_note")) || undefined,
        subtype: (type === "misc" ? fieldStr(fields, "type").toLowerCase().trim() : "") || undefined,
      };
...
/**
 * Separate featured publications from the full archive.
 *
 * Featured publications are marked with `featured = {true}` in their BibTeX
 * entry. They appear in the "Selected works" section at the top of the
 * publications index page while also remaining in the full chronological
 * archive below. This preserves the single-source-of-truth principle:
 * one .bib file drives both views.
 *
 * Returns: [featured, all] — featured pubs in their original sort order,
 * and the complete list unchanged.
 */
export function splitFeatured(pubs: Publication[]): {
  featured: Publication[];
  all: Publication[];
} {
  return {
    featured: pubs.filter((p) => p.featured),
    all: pubs,
  };
}
 ...
 *
 * For `@misc` entries, the optional `type` field is used to distinguish
 * posters, talks, and invited talks from general preprints. This follows
 * the biblatex convention: `@misc{key, type = {poster}, ...}`.
 */
export function pubTypeLabel(type: PubType, subtype?: string): string {
  /* Handle @misc subtypes for conference presentations */
  if (type === "misc" && subtype) {
    ...
    const label = subtypeMap[normalized];
